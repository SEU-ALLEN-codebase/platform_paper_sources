...
def parse_eswc(swc_file):
    ...
    with open(swc_file) as fp:
        for line in fp.readlines():
            ...
            idx, type_, x, y, z, r, p, _, _, _, _, _ = line.split()
            idx = int(idx)
            type_ = int(type_)
            x = float(x)
            y = float(y)
...
def parse_swc_upsample(swc_file):
    tree = []
    with open(swc_file) as fp:
        for line in fp.readlines():
            line = line.strip()
            ...
            idx, type_, x, y, z, r, p, _, _ = line.split()
            idx = int(idx)
            type_ = int(type_)
            x = float(x)
            y = float(y)
...
def parse_swc_upsample_forMaxR(swc_file):
    tree = []
    with open(swc_file) as fp:
        for line in fp.readlines():
            line = line.strip()
...
def parse_swc_forMaxR(swc_file):
    tree = []
    with open(swc_file) as fp:
        for line in fp.readlines():
            line = line.strip()
...
def back_in_tree(tree, imgsize=96, imgstep=8):
    new_tree = []
    for leaf in tree:
        idx, type_, x, y, z, r, p = leaf
        if abs(x-imgsize/2)>=(imgsize/2-imgstep) or abs(y-imgsize/2)>=(imgsize/2-imgstep) or abs(z-imgsize/2)>=(imgsize/2-imgstep): continue
        new_tree.append((idx, type_, x, y, z, r, p)) 
    return new_tree

def rm_point_from_tree(tree, nodes):
    new_tree = []
    for leaf in tree:
        idx, type_, x, y, z, r, p = leaf
        if idx in nodes: continue
        new_tree.append((idx, type_, x, y, z, r, p)) 
    return new_tree
...
def scalez_swc(swc_file, scale=0.5):
    if type(swc_file) == list:
        tree = swc_file
    else:
        tree = parse_swc(swc_file)
